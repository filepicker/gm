...
module.exports = function (proto) {
  ...
  function streamToUnemptyBuffer(stream, proc, callback) {
    var done = false
    var buffers = []

    var timeout = setTimeout(function(){
      done = true
      err = new Error('Operation resulted in a timeout.');
      err.httpCode = 400;
      callback(err);
      proc.stdin.pause();
      proc.kill();
  ...
  proto.toBuffer = function toBuffer (format, callback) {
    ...
    return this.stream(format, function (err, stdout, stderr, cmd, proc) {
      if (err) return callback(err);

      streamToUnemptyBuffer(stdout, proc, callback);
    })
  }
  ...
  proto._spawn = function _spawn (args, bufferOutput, callback) {
    ...
    if (self.sourceBuffer) {
      ...
      if (self.bufferStream && !this._buffering) {
        ...
        streamToUnemptyBuffer(self.sourceStream, proc, function (err, buffer) {
